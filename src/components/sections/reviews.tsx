...
export default function Reviews() {
  ...
  return (
    <section id="reviews" className="px-6 md:px-12 py-16 md:py-24">
        ...
        {reviews.map((review, index) => (
          <motion.div key={index} variants={cardVariants}>
            <Card
              ...
              role="button"
              tabIndex={0}
              aria-label={`View full review from ${review.clientName}`}
              onKeyDown={(e) => {
                if (e.key === "Enter" || e.key === " ") {
                  e.preventDefault();
                  handleReviewCardClick(index, review.clientName);
                }
              }}
            >
              <CardHeader>
                <div className="flex items-center gap-3 md:gap-2 justify-between">
                  ...
                  <Link
                    ...
                    onClick={(e) =>
                      handleLinkedInClick(e, review.clientName, review.linkedin)
                    }
                  >
                    <Button
                      size="icon"
                      className="cursor-pointer size-12 md:size-10 shrink-0"
                      aria-label={`LinkedIn profile of ${review.clientName}`}
                    >
      ...
      >
        {selectedReview !== null && (
          <DialogContent className="max-h-[calc(100vh-20rem)]">
            <DialogHeader>
              <div className="flex gap-4">
                <div className="">
                  ...
                  <DialogDescription className="text-base md:text-sm text-left">
                ...
                <Link
                  ...
                  onClick={() =>
                    handleDialogLinkedInClick(
                      reviews[selectedReview].clientName,
                      reviews[selectedReview].linkedin,
                    )
                  }
                >
                  <Button
                    size="icon"
                    className="cursor-pointer size-12 md:size-10 shrink-0"
                    aria-label={`LinkedIn profile of ${reviews[selectedReview].clientName}`}
                  >
                    <IconBrandLinkedin className="size-5 md:size-4" />
                  </Button>
                </Link>
            ...
            <ScrollArea className="h-[200px] text-base md:text-base text-foreground italic leading-relaxed">
