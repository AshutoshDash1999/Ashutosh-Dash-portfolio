...
export default function OpenSourcePage() {
    const contributions: OpenSourceContribution[] =
        (data as unknown as { openSourceContributions?: OpenSourceContribution[] }).openSourceContributions ?? [];
    ...
    return (
        <PageLayout>
            ...
            <motion.div
                ...
                animate="visible"
            >
                {contributions.map((item: OpenSourceContribution, index: number) => (
                        ...
                        <Card className="h-full hover:shadow-lg transition-shadow duration-300 bg-secondary-background flex flex-col">
                            <CardHeader>
                                ...
                                <CardDescription className="text-sm text-muted-foreground">
                                    ...
                                    >
                                        {item.org} <IconExternalLink className="size-4" aria-hidden="true" />
                                    </Link>
                                </CardDescription>
                            ...
                            <CardFooter className="flex flex-col sm:flex-row gap-2">
                                ...
                                {item.issueUrl && (
                                    <Button asChild variant="neutral" className="flex-1 bg-chart-2">
                                        <Link
                                            href={item.issueUrl}
                                            target="_blank"
                                            rel="noopener noreferrer"
